package ro.edu.ubb.servlet;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import ro.edu.ubb.entity.RoleType;
import ro.edu.ubb.entity.User;
import ro.edu.ubb.service.ServiceException;
import ro.edu.ubb.service.UserService;

/**
 * Servlet for new organizer.
 * 
 * @author Nemet Orsolya, noim1553, 532/1 csoport
 *
 */
@WebServlet("/neworganizer.do")
public class NewOrganizerServlet extends HttpServlet{

	/**
	 * 
	 */
	private static final long serialVersionUID = 8326508200066736249L;
	private UserService userService= new UserService();
	
	@Override
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		if(userService.findByUsername(req.getSession().getAttribute("loggedUsername").toString()).getRoleType()==RoleType.ADMINISTRATOR){
			dispatch("neworganizer.jsp", req, res);
		}
		else {
			dispatch("error.jsp",req,res);
		}
	}

	@Override
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		try {
			User user=new User();
			user.setFirstname(req.getParameter("firstName"));
			user.setLastname(req.getParameter("lastName"));
			user.setUsername(req.getParameter("userName"));
			user.setPassword(req.getParameter("password"));
			user.setEmail(req.getParameter("email"));
			user.setPhoneNumber(req.getParameter("phoneNumber"));
			user.setRoleType(RoleType.ORGANIZER);
			User existedUserName=userService.findByUsername(user.getUsername());
			User existedUserWithEmail=userService.findByEmail(user.getEmail());
			if(existedUserName.getUsername()==null && existedUserWithEmail.getUsername()==null) {
				userService.createCheck(user);
			}
			else {
				//TODO
			}
		}
		catch(ServiceException e) {
			dispatch("error.jsp",req,res);
		}
	}

	public void dispatch(String jsp, HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		if (jsp != null) {
			RequestDispatcher rd = req.getRequestDispatcher(jsp);
			rd.forward(req, res);
		}
	}

}

